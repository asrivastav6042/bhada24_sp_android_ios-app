...
class _AddListingServiceScreenState extends State<AddListingServiceScreen> {
  ...
  void _prefillFromService(EventServiceModel svc) {
    ...
    _primaryContactCtrl.text = _digitsOnly(svc.primaryContact ?? _primaryContactCtrl.text);
    _whatsappCtrl.text = _digitsOnly(svc.whatsappNumber ?? '');

    _businessNameCtrl.text = svc.businessName ?? svc.serviceName ?? _businessNameCtrl.text;
    ...
    _minPriceCtrl.text = (svc.minPrice ?? svc.priceMin ?? svc.price ?? '').toString();
    ...
    _imageUrls = List<String>.from(svc.imageUrls ?? const []);
  }
  ...
  Future<void> _submit() async {
    ...
    if (!_acceptedTerms) {
      showAppToast(context, 'Please accept Terms & Privacy Policy.', isError: true);
      return;
    }
    ...
    if (!mounted) return;
    if (ok) {
      showAppToast(context, _isEditMode ? 'Service updated successfully!' : 'Service created successfully!');
      context.pop();
      return;
    }
    ...
    showAppToast(
      context,
      provider.error ?? (_isEditMode ? 'Failed to update service.' : 'Failed to create service.'),
      isError: true,
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    final subCategories = _selectedCategory == null
        ? <String>[]
        : catProvider.getSubcategoriesForCategory(_selectedCategory!);

    return Scaffold(
      appBar: AppHeader(title: _isEditMode ? 'Edit Listing Service' : 'Add Listing Service'),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextFormField(
                      ...
                      decoration: const InputDecoration(hintText: '10-digit mobile', counterText: ''),
                    ),
                    const SizedBox(height: 12),
                    ...
                    DropdownButtonFormField<String>(
                      ...
                      items: subCategories
                          ...
                          .toList(),
                      onChanged: subCategories.isEmpty ? null : (v) => setState(() => _selectedSubCategory = v),
                      validator: (v) => v == null || v.isEmpty ? 'Sub-category is required' : null,
                      decoration: InputDecoration(
                        hintText: subCategories.isEmpty
                    ...
                    TextFormField(
                      ...
                      keyboardType: TextInputType.number,
                      validator: (v) {
                        if (v == null || v.trim().isEmpty) return 'Minimum price is required';
                    ...
                    Row(
                      children: [
                        ChoiceChip(
                          ...
                          onSelected: (_) => setState(() => _availableOnWeekends = true),
                        ),
                        const SizedBox(width: 8),
                        ChoiceChip(
                          label: const Text('No'),
                          selected: !_availableOnWeekends,
                          onSelected: (_) => setState(() => _availableOnWeekends = false),
                        ),
                      ],
                    ),
                    const SizedBox(height: 20),
                    ...
                    DropdownButtonFormField<String>(
                      isExpanded: true,
                      initialValue: _ownerIdProofType,
                      items: const [
                      ...
                      decoration: const InputDecoration(hintText: 'Select ID proof type'),
                    ),
                    const SizedBox(height: 12),
                    ...
                    _docTile(
                      ...
                      onUpload: () => _pickDoc(
                        ...
                        onUploaded: (url) => _insuranceCertificateUrl = url,
                      ),
                    ),
                          ...
                          child: SizedBox(
                            ...
                            child: ElevatedButton(
                              ...
                              child: _saving
                                  ...
                                  : Text(_isEditMode ? 'Update Event Service' : 'Add Event Service'),
                            ),
                          ),
                        ),
                        ...
                        Expanded(
                          child: SizedBox(
                            ...
                            child: OutlinedButton(
                              onPressed: _saving ? null : () => _formKey.currentState?.reset(),
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            value == null || value.isEmpty ? 'No document uploaded' : 'Uploaded',
            style: TextStyle(
              color: value == null || value.isEmpty ? AppColors.textSecondary : AppColors.success,
            ),
          ),
          const SizedBox(height: 8),
          SizedBox(
